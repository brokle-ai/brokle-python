...
class BrokleCallbackHandler(BaseCallbackHandler):
        ...
        try:
            ...
            if serialized:
                add_span_attributes(span, {
                    BrokleOtelSpanAttributes.LLM_CLASS: serialized.get('_type', 'unknown'),
            ...
            if self.capture_inputs and prompts:
                for i, prompt in enumerate(prompts[:5]):  # Limit to first 5 prompts
                    add_span_attributes(span, {
                        f"input.prompt.{i}": self._truncate_content(prompt)
                    })
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if serialized:
                add_span_attributes(span, {
                    BrokleOtelSpanAttributes.CHAIN_TYPE: serialized.get('_type', 'unknown'),
                    BrokleOtelSpanAttributes.CHAIN_NAME: serialized.get('name', 'unnamed')
                })
    ...
    def on_tool_start(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if serialized:
                add_span_attributes(span, {
                    BrokleOtelSpanAttributes.TOOL_NAME: serialized.get('name', 'unknown'),
                    BrokleOtelSpanAttributes.TOOL_DESCRIPTION: serialized.get('description', '')[:500]
                })
            ...
            if self.capture_inputs and input_str:
                add_span_attributes(span, {
                    BrokleOtelSpanAttributes.TOOL_INPUT: self._truncate_content(input_str)
                })
    ...
    def on_tool_end(self, output: str, **kwargs: Any) -> Any:
        ...
        try:
            ...
            if self.capture_outputs and output:
                add_span_attributes(span, {
                    BrokleOtelSpanAttributes.TOOL_OUTPUT: self._truncate_content(output)
                })
        ...
        try:
            ...
            if action:
                add_span_attributes(span, {
                    ...
                    BrokleOtelSpanAttributes.AGENT_TOOL_INPUT: self._truncate_content(str(action.tool_input)),
                    BrokleOtelSpanAttributes.AGENT_LOG: self._truncate_content(action.log)
                })
    ...
    def on_agent_finish(self, finish: AgentFinish, **kwargs: Any) -> Any:
        ...
        try:
            ...
            if finish and hasattr(finish, 'return_values'):
                add_span_attributes(span, {
                    BrokleOtelSpanAttributes.AGENT_RETURN_VALUES: self._serialize_outputs(finish.return_values)
                })

            if finish and hasattr(finish, 'log'):
                add_span_attributes(span, {
                    BrokleOtelSpanAttributes.AGENT_LOG: self._truncate_content(finish.log)
                })
    ...
    def _add_context_attributes(self, span: Any):
        ...
        try:
            ...
            if self.user_id:
                add_span_attributes(span, {
                    BrokleOtelSpanAttributes.USER_ID: self.user_id
                })

            if self.tags:
                add_span_attributes(span, {
                    BrokleOtelSpanAttributes.TAGS: ','.join(self.tags)
                })
            ...
            if hasattr(response, 'llm_output') and response.llm_output:
                ...
                if isinstance(llm_output, dict):
                    ...
                    if 'token_usage' in llm_output:
                        ...
                        add_span_attributes(span, {
                            BrokleOtelSpanAttributes.INPUT_TOKENS: usage.get('prompt_tokens', 0),
                            BrokleOtelSpanAttributes.OUTPUT_TOKENS: usage.get('completion_tokens', 0),
                            BrokleOtelSpanAttributes.TOTAL_TOKENS: usage.get('total_tokens', 0)
                        })

                    # Model name
                    if 'model_name' in llm_output:
                        add_span_attributes(span, {
                            BrokleOtelSpanAttributes.MODEL_NAME: llm_output['model_name']
                        })

            # Capture generations
            if hasattr(response, 'generations') and response.generations:
                for i, generation_list in enumerate(response.generations[:3]):  # Limit to first 3
                    for j, generation in enumerate(generation_list[:2]):  # Limit to first 2 per list
                        if hasattr(generation, 'text'):
                            add_span_attributes(span, {
                                f"output.generation.{i}.{j}": self._truncate_content(generation.text)
                            })
