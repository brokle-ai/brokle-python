...
def validate_wrapper_config(**config) -> None:
    ...
    for param in bool_params:
        if param in config and not isinstance(config[param], bool):
            raise ValidationError(f"{param} must be a boolean, got {type(config[param]).__name__}")
    ...
    if 'tags' in config and config['tags'] is not None:
        ...
        for i, tag in enumerate(tags):
            ...
            if len(tag) > 50:
                raise ValidationError(f"tags[{i}] must be <= 50 characters, got {len(tag)}")

    # Validate session_id
    if 'session_id' in config and config['session_id'] is not None:
        session_id = config['session_id']
        if not isinstance(session_id, str):
            raise ValidationError(f"session_id must be a string, got {type(session_id).__name__}")

        if len(session_id) > 100:
            raise ValidationError(f"session_id must be <= 100 characters, got {len(session_id)}")
    ...
    if 'user_id' in config and config['user_id'] is not None:
        ...
        if len(user_id) > 100:
            raise ValidationError(f"user_id must be <= 100 characters, got {len(user_id)}")