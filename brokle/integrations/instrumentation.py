    ...
    def __enter__(self):
        ...
        try:
            ...
            initial_attributes = {
                ...
                BrokleInstrumentationAttributes.OPERATION_TYPE: self.method_def['operation'],
                ...
                BrokleInstrumentationAttributes.IS_ASYNC: self.method_def.get('async', False),
                BrokleInstrumentationAttributes.STREAM_SUPPORT: self.method_def.get('stream_support', False),
            }

            # Add provider-specific request attributes
            try:
                request_attributes = self.provider.extract_request_attributes(self.kwargs)
            ...
            if hasattr(self.provider, 'config'):
                ...
                if config.get('tags'):
                    initial_attributes[BrokleInstrumentationAttributes.TAGS] = ','.join(config['tags'])
                if config.get('session_id'):
                    initial_attributes[BrokleInstrumentationAttributes.SESSION_ID] = config['session_id']
                if config.get('user_id'):
                    initial_attributes[BrokleInstrumentationAttributes.USER_ID] = config['user_id']

            # Sanitize attributes before creating span
            sanitized_attributes = AttributeValidator.sanitize_attributes(initial_attributes)
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            if self.span:
                ...
                end_attributes = {
                    BrokleInstrumentationAttributes.REQUEST_DURATION: duration * 1000,  # Convert to ms
                ...
                if self.response is not None and exc_type is None:
                    try:
                        response_attributes = self.provider.extract_response_attributes(self.response)
                ...
                if exc_type is not None:
                    ...
                    brokle_error_type = error_mapping.get(provider_error_name, 'ProviderError')
                ...
                end_attributes[BrokleInstrumentationAttributes.SUCCESS] = exc_type is None
    ...
    def instrument_client(self, client: Any) -> Any:
        ...
        try:
            ...
            if brokle_client and not brokle_client.config.telemetry_enabled:
                logger.info("Telemetry disabled in Brokle configuration. Skipping instrumentation.")
                return client
        except Exception as e:
        ...
        try:
            ...
            logger.debug(f"Instrumenting {len(methods_to_wrap)} methods for {self.provider.name}")
            ...
            logger.error(f"Critical error during {self.provider.name} instrumentation: {e}")
            raise ProviderError(f"Failed to instrument {self.provider.name} client: {e}")
        ...
        try:
            ...
            for part in path_parts[:-1]:
                if not hasattr(target_obj, part):
                    raise AttributeError(f"Path component '{part}' not found in {target_obj}")
