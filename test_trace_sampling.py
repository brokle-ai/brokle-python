...
print("1. Count total observations:")
print("   docker exec brokle-clickhouse clickhouse-client \\")
print("     --query \"SELECT COUNT(*) FROM observations WHERE name LIKE '%sampling-test%' OR name LIKE '%trace-%'\"")
print()

print("2. Verify complete traces (no orphaned children):")
print("   docker exec brokle-clickhouse clickhouse-client \\")
print("     --query \"SELECT trace_id, COUNT(*) as span_count FROM observations WHERE name LIKE '%trace-%' GROUP BY trace_id ORDER BY span_count DESC FORMAT Pretty\"")
print()

print("3. Expected results:")
print(f"   - Total observations: ~{int(NUM_TRACES * SAMPLE_RATE * 3)} spans")
...
print("   docker exec brokle-clickhouse clickhouse-client \\")
print("     --query \"SELECT trace_id, groupArray(name) as spans FROM observations WHERE name LIKE '%trace-%' GROUP BY trace_id HAVING COUNT(*) != 3 FORMAT Vertical\"")
print()
