...
def test_sdk_initialization(api_key: str, host: str) -> Optional[Any]:
    ...
    try:
        ...
        client = Brokle(
            api_key=api_key, host=host, environment="integration-test", timeout=30.0
        )
...
def main():
    ...
    with client:
        test_results.append(
            ("Raw API Communication", test_raw_api_communication(client))
        )
    ...
    if failed == 0:
        print_banner(
            "🎉 All Tests Passed! SDK ↔ Backend Communication CONFIRMED!", "🎉"
        )
